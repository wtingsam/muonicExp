...
// ********************************************************************
//
// $Id: ScintHit.cc 101036 2016-11-04 09:00:23Z gcosmo $
//
/// \file ScintHit.cc
...
ScintHit::ScintHit(G4int id,G4double time)
: G4VHit(), 
...
{
  G4bool isNew;
  auto store = G4AttDefStore::GetInstance("ScintHit",isNew);
...
std::vector<G4AttValue>* ScintHit::CreateAttValues() const
{
  ...
  values
    ->push_back(G4AttValue("HitType","ScintHit",""));
  values
...
void ScintHit::Print()
{
  G4cout << "  Scint[" << fId << "] " << fTime/ns << " (nsec)" << G4endl;
}
